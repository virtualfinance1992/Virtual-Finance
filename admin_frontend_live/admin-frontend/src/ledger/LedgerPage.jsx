...
const LedgerPage = () => {
  ...
  const fetchGroups = async (companyId) => {
    try {
      const res = await fetch(`http://127.0.0.1:8000/api/accounting/account-groups/${companyId}/`, {
  ...
  const fetchLedgers = async () => {
    ...
    try {
      const res = await axios.get(
        `http://127.0.0.1:8000/api/accounting/ledger/list/${selectedCompany.id}/`,
        {
          headers: {
  ...
  const handleLedgerCreate = async () => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:8000/api/accounting/ledger/create/`, {
        method: 'POST',
  ...
  const handleEditSave = async () => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:8000/api/accounting/ledger/update/${activeLedger.id}/`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify(payload)
      });
  ...
  const handleDeleteLedger = async () => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:8000/api/accounting/ledger/delete/${activeLedger.id}/`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
