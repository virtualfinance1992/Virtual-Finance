...
const PurchaseVoucherForm = ({ onClose, companyId }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const res1 = await fetch(`http://localhost:8000/api/inventory/items/${companyId}/`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        const invData = await res1.json();
        setInventoryList(Array.isArray(invData) ? invData : []);

        const res2 = await fetch(`http://localhost:8000/api/suppliers/${companyId}/`, {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const handleSavePurchase = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:8000/api/vouchers/purchase/${companyId}/create/`, {
        method: 'POST',
        headers: {
