...
const JournalEntryForm = ({ onClose, companyId }) => {
  ...
  useEffect(() => {
    const fetchVoucherNumber = async () => {
      try {
        const res = await fetch(`http://localhost:8000/api/vouchers/journal/next-number/?company_id=${resolvedCompanyId}`);
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:8000/api/vouchers/journal/${resolvedCompanyId}/create/`, {
        method: 'POST',
