// ✅ IncomePartyForm.jsx - Modal popup to add a new Income Party
import React, { useState } from 'react';

const IncomePartyForm = ({ onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`https://virtual-finance-backend.onrender.com/api/incomes/party/create/${companyId}/`, {
        method: 'POST',
        headers: {
      ...
      alert('✅ Income party created successfully.');
      onClose();
    } catch (err) {
      console.error('❌ Error saving income party:', err);
      alert('❌ Failed to save income party.');
    }
  };
  ...
  return (
    <div style={{ background: '#fff', padding: '24px', borderRadius: '12px', width: '500px', fontFamily: 'Arial, sans-serif', boxShadow: '0 16px 32px rgba(0,0,0,0.25)' }}>
      <h2 style={{ color: '#003366', marginBottom: '20px', textAlign: 'center' }}>➕ Add Income Party</h2>
...
export default IncomePartyForm;
